...
public class Images {
	...
	private static Image load(String resourcePath) {
	    if (resourcePath == null) {
	        logger.logError("La ruta del recurso de imagen es null.");
	    ...
	    try (var stream = Images.class.getResourceAsStream(resourcePath)) {
	        ...
	        //logger.logDebug("Imagen cargada con éxito: " + resourcePath);
	        return image;

	    } catch (Exception e) {
	        logger.logError("Excepción al cargar la imagen '" + resourcePath + "': " + e.getMessage());
	        return null;
	    }
	}
	...
	public static final Image IMG_BUTTONS_FLECHA_DERECHA = load(Paths.IMG_BUTTONS_FLECHA_DERECHA_PNG);
	...
	public static final Image IMG_FAVICONS_LOG = load(Paths.IMG_FAVICONS_LOG_PNG);
