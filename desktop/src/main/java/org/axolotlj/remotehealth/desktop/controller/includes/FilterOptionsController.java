...
import org.axolotlj.remotehealth.core.validations.FilterValidation;
...
public class FilterOptionsController {
	...
	private void saveToAnalysisEcg() {
	    ...
	    if (AlertUtil.handleValidation(FilterValidation.validateOrder(bpOrder))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateFrequency(bpLow))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateFrequency(bpHigh))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateInferiorLessThanSuperior(bpLow, bpHigh))) return;

	    if (AlertUtil.handleValidation(FilterValidation.validateOrder(bsOrder))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateFrequency(bsLow))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateFrequency(bsHigh))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateInferiorLessThanSuperior(bsLow, bsHigh))) return;

	    if (AlertUtil.handleValidation(FilterValidation.validateWaveletLevel(waveletLevel))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateThreshold(waveletThreshold))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validateWindowSize(sgWindow))) return;
	    if (AlertUtil.handleValidation(FilterValidation.validatePolynomial(sgPoly, sgWindow))) return;
