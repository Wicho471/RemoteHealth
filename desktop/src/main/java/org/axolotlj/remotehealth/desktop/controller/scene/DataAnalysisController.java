...
public class DataAnalysisController implements DisposableController {
	// Campos no FXML
	...
	public void initialize() {
		ChartUtils.setStyle(ecgChart, DesktopPaths.CSS_DASHBOARDSTYLE_CSS);
		ChartUtils.setStyle(ppgChart, DesktopPaths.CSS_DASHBOARDSTYLE_CSS);
		ChartUtils.setStyle(bpmChart, DesktopPaths.CSS_DASHBOARDSTYLE_CSS);
		ChartUtils.setStyle(tempChart, DesktopPaths.CSS_DASHBOARDSTYLE_CSS);
	...
	private void selectFileHandler() {
		FileChooserUtils
				.chooseFile(SceneManager.getStage(), "Selecciona archivo CSV", "Archivos CSV", "*.csv")
	...
	public void loadFile(String pathString) {
		...
		loadTask.setOnFailed(event -> {
		    ...
		    if (throwable instanceof Exception exception) {
		        dataLogger.logException("Error al cargar archivo", exception);
		    } else {
		        dataLogger.logFatal("Error crítico no manejado: " + throwable.getClass().getName());
		    }
		});
...
//
						...
						e.printStackTrace();
						dataLogger.logException("Error al plotear datos adicionales: ", e);
					}
