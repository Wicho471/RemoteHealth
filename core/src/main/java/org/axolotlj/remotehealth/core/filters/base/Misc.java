...
public class Misc {
	public static double[] normalize(double[] values) {
	    ...
	    if (range == 0) {
	    	Log.get().logWarn("⚠️ ECG rango 0: todos los valores iguales.");
	    ...
	    if (std == 0) {
	    	Log.get().logWarn("⚠️ Desviación estándar 0: datos sin variación.");
	        return values.clone();
	    }
	...
	public static double mean(double[] data) {
		...
		public static double calculateAverageSamplingRate(long[] timestamps) {
		    if (timestamps.length < 2) {
		    	Log.get().logFatal("Se necesitan al menos 2 timestamps para calcular la tasa de muestreo.");
