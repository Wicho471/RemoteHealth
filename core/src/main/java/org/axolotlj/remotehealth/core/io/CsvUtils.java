...
public class CsvUtils {
	...
	public static List<String> getAllLines(String path) {
		if(!path.endsWith(".csv")) {
			dataLogger.logWarn("El archivo no es csv");
    	...
    	try (InputStream is = CsvUtils.class.getClassLoader().getResourceAsStream(path)) {
    	    if (is != null) {
    	        ...
    	        dataLogger.logWarn("Archivo CSV no encontrado en recursos '");
    	        return new ArrayList<>();
    	    }
    	} catch (IOException e) {
    		dataLogger.logException("Ocurrio un error", e);
    	    return new ArrayList<>();
    	}
    	return list;
	}
		...
		for (File file : files) {
			...
			if (fileName.toLowerCase().endsWith(".csv")) {
				...
				try {
					...
					dataLogger.logException("Error al descomprimir archivo: '" + fileName + "'", e);
				}
			}
		}
