package org.axolotlj.remotehealth.core.analysis.hr;
...
public class HrMonitor {
    ...
    private final Runnable beatCallback;
     ...
     */
    public HrMonitor(double fs, double beatsInWindow,
                     Consumer<ImmutablePair<Long, Integer>> listener,
                     Runnable beatCallback,
                     Consumer<String> anomalyCallback) {
        this.integWin = (int) Math.round(0.15 * fs);
     ...
     * cardíaca.
     */
    public HrMonitor(Consumer<ImmutablePair<Long, Integer>> listener,Runnable beatCallback,
                     Consumer<String> anomalyCallback) {
        this.integWin = (int) Math.round(0.15 * 250);
        this.beatsInWindow = 5;
        this.listener = listener;
        this.anomalyTrigger = new AnomalyTrigger(
                10000,        // 3s persistencia
                60_000,      // 1min enfriamiento
                anomalyCallback
        );
        this.beatCallback = beatCallback;
    }
    ...
    public void addEcgSample(double ecg, long tMs) {
        ...
        if (isPeak && isNewBeat(tMs)) {
            ...
            if (beatCallback != null) beatCallback.run();
        }
        ...
        int roundedBpm = (int) Math.round(bpm);
        listener.accept(ImmutablePair.of(tMs, roundedBpm));

        boolean isAnomaly = (roundedBpm > 120 || roundedBpm < 50);
        if (anomalyTrigger != null) {
            String msg = roundedBpm > 120
                    ? "Taquicardia detectada: " + roundedBpm + " bpm"
                    : "Bradicardia detectada: " + roundedBpm + " bpm";
            anomalyTrigger.evaluate(isAnomaly, System.currentTimeMillis(), msg);
        }
    }
